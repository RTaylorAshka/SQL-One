...
-- SELECT * FROM owners JOIN vehicles ON owner_id = owners.id;

-- Count the number of cars for each owner. Display the owners first_name, last_name and count of vehicles.
-- SELECT first_name, last_name, COUNT(owner_id) AS count FROM owners JOIN vehicles ON owner_id = owners.id GROUP BY owners.id ORDER BY count;
...
--SELECT first_name, last_name, ROUND(AVG(price)) AS avg_price, COUNT(owner_id) AS count FROM owners 
--JOIN vehicles ON owner_id = owners.id  GROUP BY owners.id HAVING (COUNT(owner_id) > 1) AND (ROUND(AVG(price))) > 10000 ORDER BY count DESC;